...
logging.basicConfig(
    ...
    handlers=[logging.StreamHandler(), logging.FileHandler("notifications.log")],
)
...
class NotificationService:
    ...
    def get_scheduler(cls) -> AsyncIOScheduler:
        ...
        logger.info(
            f"[DEBUG] Current scheduler state: running={cls._scheduler_instance.running}, jobs={len(jobs)}"
        )
        for job in jobs:
    ...
    async def _schedule_user_notifications(self, settings: NotificationSettings):
        ...
        if not settings.enabled:
            ...
            logger.info(
                f"Removing jobs for disabled notifications (user {settings.user_id})"
            )
    ...
    async def _send_notifications(self, user_id: int):
        ...
        try:
            ...
            logger.info(
                f"[DEBUG] Settings for user {user_id}: {settings.enabled if settings else None}"
            )

            if not settings:
                logger.info(
                    f"[DEBUG] No notification settings found for user {user_id}"
                )
                return
            ...
            logger.info(
                f"[DEBUG] User {user_id} found: {user is not None}, role: {user.role if user else None}"
            )
            ...
            logger.info(
                f"[DEBUG] Processing notifications for user {user_id} with role {user.role}"
            )
            ...
            if user.role == "owner":
                ...
                logger.info(
                    f"[DEBUG] Found {len(accounts) if accounts else 0} accounts for owner {user_id}"
                )
            else:
                ...
                try:
                    ...
                    logger.info(
                        f"[DEBUG] Found {len(shared_accounts)} shared accounts for user {user_id}"
                    )
                    ...
                    logger.info(
                        f"[DEBUG] Using owner's token (ID: {owner_id}) for API requests"
                    )
                        ...
                        try:
                            ...
                            account_data = next(
                                (
                                    ...
                                    if acc["id"] == account.fb_account_id
                                ),
                                None,
                            )
                            ...
                            logger.error(
                                f"[DEBUG] Error getting data for account {account.fb_account_id}: {str(e)}"
                            )

                    logger.info(
                        f"[DEBUG] Successfully retrieved data for {len(accounts)} accounts"
                    )
            ...
            logger.info(
                f"[DEBUG] Starting to process {len(accounts)} accounts for user {user_id}"
            )
            ...
            for account in accounts:
                ...
                logger.info(
                    f"[DEBUG] Processing account {account_id} ({account_name}) for user {user_id}"
                )
                ...
                try:
                    ...
                    logger.info(
                        f"[DEBUG] Got analysis for account {account_id}, sending notification"
                    )
                    ...
                    for part in message_parts:
                        ...
                        logger.info(
                            f"[DEBUG] Sent notification part for account {account_id} to user {user_id}"
                        )

                except Exception as e:
                    logger.error(
                        f"[DEBUG] Error processing account {account_id}: {str(e)}"
                    )
                    continue
