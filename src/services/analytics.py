...
class AnalyticsService:
    ...
    async def get_comparative_insights(
        self, user_id: int, account_id: str, period: ComparisonPeriod
    ) -> Tuple[Dict, Dict]:
        """
        ...
        try:
            response = await self.openai_client.chat.completions.create(
                ...
                messages=[
                    {
                        "role": "system",
                        "content": "Ты - опытный специалист по Facebook рекламе. Анализируешь статистику и даешь конкретные рекомендации по улучшению.",
                    },
    ...
    async def get_comprehensive_analysis(
        ...
        periods = [
            ...
            (
                ComparisonPeriod.BIWEEKLY,
                "monthly",
                True,
            ),  # Нормализуем двухнедельные данные
        ]
        ...
        try:
            ...
            if recent_insights:
                ...
                logger.info(
                    f"Account {account_id} activity check: spend={total_spend}, conversions={total_conversions}"
                )
                ...
                if total_spend <= 0 or total_conversions <= 0:
                    logger.info(
                        f"Account {account_id} marked as inactive: no spend or conversions in last 2 weeks"
                    )
                    return None
            else:
                logger.info(
                    f"Account {account_id} marked as inactive: no insights data"
                )
                return None

        except Exception as e:
            logger.error(
                f"Ошибка при проверке активности аккаунта {account_id}: {str(e)}"
            )
            return None
        ...
        for period, period_type, should_normalize in periods:
            try:
                current_insights, previous_insights = (
                    await self.get_comparative_insights(user_id, account_id, period)
                )
