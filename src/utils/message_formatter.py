...
from src.utils.languages import get_language, get_text
...
def format_insights(
    ...
    if total_impressions > 0:
        ...
        message += (
            f"• <b>{get_text('cpm', lang)}:</b> {format_currency(cpm, currency)}\n"
        )

    if total_clicks > 0:
        cpc = total_spend / total_clicks
        message += (
            f"• <b>{get_text('cpc', lang)}:</b> {format_currency(cpc, currency)}\n"
        )
    ...
    for insight in insights:
        if (
            "actions" in insight
            or "conversions" in insight
            or "cost_per_action_type" in insight
        ):
    ...
    if conversion_data:
        ...
        for insight in insights:
            ...
            for conversion in conversions:
                ...
                if action_type.startswith("offsite_conversion.fb_pixel_custom."):
                    ...
                    base_type = ".".join(
                        action_type.split(".")[:-1]
                    )  # Get offsite_conversion.fb_pixel_custom
        ...
        conversion_labels = {
            ...
            "offsite_conversion.fb_pixel_lead": get_text(
                "offsite_conversion.fb_pixel_lead", lang
            ),
            "offsite_conversion.fb_pixel_purchase": get_text(
                "offsite_conversion.fb_pixel_purchase", lang
            ),
        }
...
def format_campaign_table(
    ...
    for campaign in campaigns:
        ...
        if campaign_insights:
            ...
            logger.info(
                f"Кампания {campaign_name} ({campaign_id}): получены сырые данные insights: {json.dumps(campaign_insights, indent=2)}"
            )

            # Sum up metrics across all insights for this campaign
            total_impressions = sum(
                float(i.get("impressions", 0)) for i in campaign_insights
            )
            ...
            for insight in campaign_insights:
                ...
                for conversion in conversions:
                    ...
                    logger.info(
                        f"Кампания {campaign_name} ({campaign_id}): найдена конверсия {action_type} = {value}"
                    )
                ...
                for cost in costs:
                    ...
                    logger.info(
                        f"Кампания {campaign_name} ({campaign_id}): найдена стоимость конверсии {action_type} = {value}"
                    )
                ...
                for conversion in conversions:
                    ...
                    if action_type.startswith("offsite_conversion.fb_pixel_custom."):
                        ...
                        if conversion_name in custom_conversion_types:
                            ...
                            logger.info(
                                f"Используем конверсию {action_type} со значением {value}"
                            )
                            if value > 0:
                                had_activity = True
                            break
                        # Если конкретное название не найдено, сохраняем первую найденную кастомную конверсию
                            ...
                            custom_conversion_found = True
                            logger.info(
                                f"Используем первую найденную кастомную конверсию {action_type} со значением {value}"
                            )
                            if value > 0:
                                had_activity = True
                ...
                for insight in campaign_insights:
                    ...
                    for conversion in conversions:
                        ...
                        if action_type in ["lead", "offsite_conversion.fb_pixel_lead"]:
                            results += value
                            logger.info(
                                f"Используем стандартную конверсию {action_type} со значением {value}"
                            )
                            if value > 0:
                                had_activity = True
            ...
            for insight in campaign_insights:
                costs = insight.get("cost_per_action_type", [])
                logger.info(
                    f"Кампания {campaign_name} ({campaign_id}): все стоимости конверсий: {json.dumps(costs, indent=2)}"
                )
                for cost in costs:
                    action_type = cost.get("action_type", "")
                    if action_type == "offsite_conversion.fb_pixel_custom":
                        ...
                        logger.info(
                            f"Используем стоимость базовой кастомной конверсии {action_type} со значением {cost_per_conversion}"
                        )
                        break
                ...
                for insight in campaign_insights:
                    ...
                    for cost in costs:
                        ...
                        for conv_type in custom_conversion_types:
                            if (
                                action_type
                                == f"offsite_conversion.fb_pixel_custom.{conv_type}"
                            ):
                                cost_per_conversion = float(cost.get("value", 0))
                                used_conversion_type = action_type
                                custom_cost_found = True
                                logger.info(
                                    f"Используем стоимость конверсии {action_type} со значением {cost_per_conversion}"
                                )
                                break
                ...
                for insight in campaign_insights:
                    ...
                    for cost in costs:
                        ...
                        if action_type.startswith(
                            ...
                            custom_cost_found = True
                            logger.info(
                                f"Используем стоимость для другой кастомной конверсии {action_type} со значением {cost_per_conversion}"
                            )
                            break
                ...
                for insight in campaign_insights:
                    ...
                    for cost in costs:
                        ...
                        if action_type in ["lead", "offsite_conversion.fb_pixel_lead"]:
                            ...
                            used_conversion_type = action_type
                            logger.info(
                                f"Используем стоимость стандартного лида {action_type} со значением {cost_per_conversion} только потому, что не нашли кастомных конверсий"
                            )
                            break
    ...
    weighted_frequency = (
        sum(f * w for f, w in weighted_frequency_data)
        / sum(w for _, w in weighted_frequency_data)
        if weighted_frequency_data
        else 0
    )
    weighted_ctr = (
        sum(c * w for c, w in weighted_ctr_data) / sum(w for _, w in weighted_ctr_data)
        if weighted_ctr_data
        else 0
    )
    weighted_cpm = (
        sum(c * w for c, w in weighted_cpm_data) / sum(w for _, w in weighted_cpm_data)
        if weighted_cpm_data
        else 0
    )
    weighted_cost_per_conversion = (
        sum(c * w for c, w in weighted_cost_per_conversion_data)
        / sum(w for _, w in weighted_cost_per_conversion_data)
        if weighted_cost_per_conversion_data
        else 0
    )

    # Определяем самый часто используемый тип конверсии
    most_common_conversion_type = (
        max(conversion_type_counts.items(), key=lambda x: x[1])[0]
        if conversion_type_counts
        else "Нет"
    )
