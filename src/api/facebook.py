...
from config.settings import DATE_PRESETS, FB_API_VERSION
...
class FacebookAdsApiError(Exception):
    ...
    def __init__(
        self, message: str, code: Optional[str] = None, data: Optional[Dict] = None
    ):
...
class FacebookAdsClient:
        ...
        while retry_count < retries:
            try:
                ...
                if "error" in data:
                    ...
                    logger.error(
                        f"Facebook API error: {error_message} (code: {error_code}, type: {error_type}, subcode: {error_subcode})"
                    )
                    ...
                    if error_code == 190 or error_type == "OAuthException":
                        if (
                            ...
                            internal_error_code = (
                                "TOKEN_EXPIRED"  # Общий случай для OAuth ошибок
                            )
                        print(
                            f"DEBUG: OAuth error detected: {error_message}. Setting internal_error_code to {internal_error_code}"
                        )
    ...
    async def get_ad_accounts(self) -> List[Dict[str, Any]]:
        ...
        try:
            ...
            print(
                f"DEBUG: Owner's token validation - Current time: {datetime.utcnow().isoformat()}, "
            ...
            print(
                f"DEBUG: Making API request to: {url}?fields={params['fields']}&access_token=REDACTED_TOKEN"
            )
                ...
                async with session.get(url, params=params) as response:
                    if response.status != 200:
                        ...
                        raise FacebookAdsApiError(
                            f"Failed to get ad accounts: {error_message}", "API_ERROR"
                        )

                    data = await response.json()
                    accounts = data.get("data", [])
                    print(
                        f"DEBUG: Found {len(accounts)} ad accounts for user {self.user_id}"
                    )
                    return accounts
        finally:
            session.close()
    ...
    async def get_accounts(self) -> Tuple[List[Dict], Optional[str]]:
        ...
        try:
            ...
            if e.code == "TOKEN_EXPIRED":
                return (
                    [],
                    "Your access token has expired. Please use /auth to authenticate again.",
                )
                ...
                return (
                    [],
                    "You are not authenticated with Facebook. Please use /auth to authenticate.",
                )
            else:
    ...
    async def get_campaigns(self, account_id: str, limit: int = 100) -> List[Dict]:
        ...
        """
        print(
            f"DEBUG: get_campaigns called for account {account_id}, user {self.user_id}"
        )
            ...
            while retry_count <= max_retries:
                try:
                    ...
                    data = await self._make_request(
                        f"{account_id}/campaigns", {"fields": fields, "limit": limit}
                    )
                    ...
                    if "data" not in data:
                        print(
                            f"DEBUG: No 'data' field in API response: {json.dumps(data)}"
                        )
                        if retry_count < max_retries:
                            retry_count += 1
                            await asyncio.sleep(1)
                            print(
                                f"DEBUG: Retrying request ({retry_count}/{max_retries})"
                            )
                            continue
                        else:
                    ...
                    print(
                        f"DEBUG: Facebook API error in get_campaigns: {e.message} (code: {e.code})"
                    )

                    # Если ошибка связана с токеном, не пытаемся повторить
                    if (
                        e.code == "TOKEN_EXPIRED"
                        or e.code == "TOKEN_NOT_SET"
                        or e.code == "USER_NOT_FOUND"
                    ):
                        raise
                    ...
                    if retry_count < max_retries:
                        ...
                        await asyncio.sleep(
                            2
                        )  # Немного подождем перед повторной попыткой
    ...
    async def get_adsets(self, campaign_id: str, limit: int = 100) -> List[Dict]:
        ...
        try:
            ...
            data = await self._make_request(
                f"{campaign_id}/adsets", {"fields": fields, "limit": limit}
            )
    ...
    async def get_ads(self, campaign_id: str, limit: int = 100) -> List[Dict]:
        ...
        try:
            ...
            data = await self._make_request(
                f"{campaign_id}/ads", {"fields": fields, "limit": limit}
            )
                ...
                processed_ads.append(
                    {
                        ...
                        "thumbnail_url": (
                            creative.get("thumbnail_url") if creative else None
                        ),
    ...
    async def get_insights(
        ...
        if start_date and end_date:
            ...
            if not facebook_date_preset:
                logger.warning(
                    f"Invalid date preset '{date_preset}', defaulting to 'last_7_days'"
                )
                date_preset = "last_7d"
                facebook_date_preset = DATE_PRESETS.get(date_preset)
        ...
        if not fields:
            ...
            fields = [
                ...
                "date_start",
                "date_stop",  # Добавляем поля для дат
            ]
        ...
        try:
            ...
            if insights:
                aggregated_insight = {
                    ...
                    "impressions": sum(
                        float(i.get("impressions", 0)) for i in insights
                    ),
                ...
                for insight in insights:
                    for conv in insight.get("conversions", []):
                        ...
                        if action_type and action_type.startswith(
                            "offsite_conversion.fb_pixel_custom"
                        ):
                            if action_type not in conversion_values:
                                conversion_values[action_type] = 0
                            conversion_values[action_type] += float(
                                conv.get("value", 0)
                            )
                ...
                for action_type, values in cost_per_action.items():
                    if values:
                        ...
                        aggregated_insight["cost_per_action_type"].append(
                            {"action_type": action_type, "value": avg_cost}
                        )
