...
async def process_auth_code(message: Message, state: FSMContext):
    ...
    try:
        ...
        try:
            ...
            user.set_fb_token(
                token_data["access_token"], int(token_data.get("expires_in", 0))
            )
            ...
            if saved_user and saved_user.is_token_valid():
                print(
                    f"DEBUG: Verification successful - user {user_id} has valid token"
                )
            else:
                print(
                    f"DEBUG: Verification FAILED - user {user_id} token not saved properly"
                )
