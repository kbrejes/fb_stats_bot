...
def build_notification_keyboard(enabled: bool = True) -> InlineKeyboardBuilder:
    ...
    builder.button(text="🕑 Изменить время", callback_data="notifications:set_time")

    # Кнопка настройки часового пояса
    builder.button(text="🌍 Часовой пояс", callback_data="notifications:set_timezone")
...
def build_timezone_keyboard() -> InlineKeyboardBuilder:
    ...
    for display_name, tz_name in TIMEZONE_MAPPING.items():
        builder.button(text=display_name, callback_data=f"timezone:{tz_name}")

    # Кнопка отмены
    builder.button(text="🔙 Отмена", callback_data="timezone:cancel")
    ...
    try:
        ...
        if not settings:
            ...
            settings = await notification_service.create_user_notifications(
                user_id, time(10, 0), "UTC"  # 10:00 по умолчанию
            )
    ...
    try:
        ...
        if action == "enable":
            ...
            settings = await notification_service.create_user_notifications(
                ...
                enabled=True,
                notification_types=(
                    current_settings.notification_types if current_settings else None
                ),
            )
        ...
        await callback.answer(
            "❌ Произошла ошибка. Пожалуйста, попробуйте позже.", show_alert=True
        )
    finally:
        session.close()
...
async def process_notification_time(message: Message, state: FSMContext):
    ...
    try:
        ...
        settings = await notification_service.create_user_notifications(
            ...
            notification_time,
            (
                settings.timezone if settings else "UTC"
            ),  # Используем текущий часовой пояс или UTC по умолчанию
            (
                settings.enabled if settings else True
            ),  # Используем текущий статус или True по умолчанию
            (
                settings.notification_types if settings else None
            ),  # Используем текущие типы уведомлений или None для значений по умолчанию
        )
