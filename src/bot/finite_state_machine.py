...
async def cmd_start(message: Message, state: FSMContext):
    ...
    try:
        ...
        if user:
            ...
            if user.role == "owner":
                ...
                accounts_str = (
                    "\n• ".join(account_names)
                    if account_names
                    else "нет привязанных аккаунтов"
                )
        ...
        admin_users = (
            session.query(User).filter(User.role.in_(["owner", "admin"])).all()
        )
        admin_message = (
        ...
        builder.add(
            InlineKeyboardButton(
                text="✅ Принять", callback_data=f"new_user:accept:{user_id}"
            )
        )
        builder.add(
            InlineKeyboardButton(
                text="❌ Отклонить", callback_data=f"new_user:reject:{user_id}"
            )
        )
        builder.adjust(2)
        ...
        for admin in admin_users:
            try:
                ...
                await message.bot.send_message(
                    admin.telegram_id, admin_message, reply_markup=builder.as_markup()
                )
...
async def process_new_user_action(callback: CallbackQuery, state: FSMContext):
    ...
    logger.info(
        f"[NEW_USER_ACTION] 🎯 Обработка действия '{action}' от админа {admin_id} для пользователя {user_id}"
    )

    # Создаем новый state для пользователя
    new_key = StorageKey(bot_id=callback.bot.id, chat_id=user_id, user_id=user_id)
    ...
    logger.debug(
        f"[NEW_USER_ACTION] 📊 Ожидаемое состояние: {NewUserStates.waiting_for_admin.state}"
    )

    if current_state != NewUserStates.waiting_for_admin.state:
        logger.error(
            f"[NEW_USER_ACTION] ❌ Несоответствие состояния. Текущее: {current_state}, "
            f"Ожидаемое: {NewUserStates.waiting_for_admin.state}"
        )
        await callback.message.edit_text(
            "❌ Ошибка: неверное состояние процесса регистрации."
        )
    ...
    if not new_user_data or new_user_data["telegram_id"] != user_id:
        logger.error(
            f"[NEW_USER_ACTION] ❌ Несоответствие данных пользователя. "
            f"Ожидаемый user_id: {user_id}, Полученные данные: {new_user_data}"
        )
        await callback.message.edit_text(
            "❌ Ошибка: данные пользователя не найдены или не совпадают."
        )
        await user_state.clear()
        logger.debug(f"[NEW_USER_ACTION] 🗑 State пользователя очищен")
        return
    ...
    if action == "accept":
        logger.info(
            f"[NEW_USER_ACTION] ✅ Админ принял пользователя {user_id}, получаем список аккаунтов"
        )
        # Получаем список аккаунтов админа
        accounts = await get_accounts(
            admin_id
        )  # Используем admin_id для получения списка аккаунтов

        if not accounts:
            logger.error(
                f"[NEW_USER_ACTION] ❌ Не удалось получить список аккаунтов для админа {admin_id}"
            )
            await callback.message.edit_text(
        ...
        for account in accounts:
            ...
            logger.debug(
                f"[NEW_USER_ACTION] 🔘 Добавлена кнопка для аккаунта: {account_name} ({account_id})"
            )

        builder.add(
            InlineKeyboardButton(
                text="❌ Отмена", callback_data=f"assign_account:{user_id}:cancel"
            )
        )
        ...
        if new_state != NewUserStates.selecting_account.state:
            logger.error(
                f"[NEW_USER_ACTION] ❌ Не удалось установить состояние. "
                f"Текущее: {new_state}, Ожидаемое: {NewUserStates.selecting_account.state}"
            )
            await callback.message.edit_text(
                "❌ Ошибка: не удалось установить состояние выбора аккаунта."
            )
            await user_state.clear()
            logger.debug(f"[NEW_USER_ACTION] 🗑 State пользователя очищен")
            return
        ...
        logger.info(
            f"[NEW_USER_ACTION] 🔄 Переход к выбору аккаунта для пользователя {user_id}"
        )
        ...
        logger.debug(
            f"[NEW_USER_ACTION] 📨 Отправлено уведомление пользователю {user_id} об отклонении"
        )
...
async def process_account_selection(callback: CallbackQuery, state: FSMContext):
    ...
    admin_id = callback.from_user.id
    logger.info(
        f"[ACCOUNT_SELECTION] 🎯 Обработка выбора аккаунта {account_id} админом {admin_id} для пользователя {user_id}"
    )
    ...
    user_state = FSMContext(storage=state.storage, key=new_key)
    logger.debug(
        f"[ACCOUNT_SELECTION] 🔑 Создан state пользователя с ключом: {new_key}"
    )
    ...
    logger.debug(
        f"[ACCOUNT_SELECTION] 📊 Ожидаемое состояние: {NewUserStates.selecting_account.state}"
    )

    if current_state != NewUserStates.selecting_account.state:
        logger.error(
            f"[ACCOUNT_SELECTION] ❌ Несоответствие состояния. Текущее: {current_state}, "
            f"Ожидаемое: {NewUserStates.selecting_account.state}"
        )
        await callback.answer(
            "❌ Ошибка: неверное состояние процесса регистрации.", show_alert=True
        )
        return
    ...
    new_user_data = state_data.get("new_user_data")
    logger.debug(
        f"[ACCOUNT_SELECTION] 📝 Получены данные пользователя: {new_user_data}"
    )

    if not new_user_data or new_user_data["telegram_id"] != user_id:
        logger.error(
            f"[ACCOUNT_SELECTION] ❌ Несоответствие данных пользователя. "
            f"Ожидаемый user_id: {user_id}, Полученные данные: {new_user_data}"
        )
        await callback.message.edit_text(
            "❌ Ошибка: данные пользователя не найдены или не совпадают."
    ...
    if account_id == "cancel":
        logger.info(
            f"[ACCOUNT_SELECTION] ❌ Выбор аккаунта отменен для пользователя {user_id}"
        )
        await callback.message.edit_text(
    ...
    logger.debug(
        f"[ACCOUNT_SELECTION] 💾 Сохранен выбранный аккаунт {account_id} для пользователя {user_id}"
    )
    ...
    for role in roles:
        builder.add(
            InlineKeyboardButton(
                text=role, callback_data=f"assign_role:{user_id}:{role}"
            )
        )
    ...
    builder.add(
        InlineKeyboardButton(
            text="❌ Отмена", callback_data=f"assign_role:{user_id}:cancel"
        )
    )
    ...
    if new_state != NewUserStates.selecting_role.state:
        logger.error(
            f"[ACCOUNT_SELECTION] ❌ Не удалось установить состояние. "
            f"Текущее: {new_state}, Ожидаемое: {NewUserStates.selecting_role.state}"
        )
        await callback.message.edit_text(
            "❌ Ошибка: не удалось установить состояние выбора роли."
        )
        await user_state.clear()
        logger.debug(f"[ACCOUNT_SELECTION] 🗑 State пользователя очищен")
        return
    ...
    logger.info(
        f"[ACCOUNT_SELECTION] 🔄 Переход к выбору роли для пользователя {user_id}"
    )
...
async def process_role_selection(callback: CallbackQuery, state: FSMContext):
    ...
    try:
        ...
        admin_id = callback.from_user.id
        logger.info(
            f"[ROLE_SELECTION] 🎯 Обработка выбора роли '{role}' админом {admin_id} для пользователя {user_id}"
        )
        ...
        if current_state != NewUserStates.selecting_role.state:
            await callback.answer(
                "❌ Ошибка: неверное состояние процесса назначения роли.",
                show_alert=True,
            )
            return
        ...
        try:
            ...
            if existing_user:
                logger.error(
                    f"[ROLE_SELECTION] ❌ Пользователь {user_id} уже существует в БД"
                )
                await callback.message.edit_text(
            ...
            try:
                ...
                account_name = next(
                    (acc["name"] for acc in fb_accounts if acc["id"] == fb_account_id),
                    None,
                )
            except Exception as e:
                logger.error(
                    f"[ROLE_SELECTION] ❌ Ошибка при получении названия аккаунта: {str(e)}"
                )
            ...
            exists = (
                session.query(accounts_to_users)
            ...
            await notification_service._schedule_user_notifications(
                ...
                if welcome_message_id and welcome_chat_id:
                    try:
                        await callback.bot.delete_message(
                            welcome_chat_id, welcome_message_id
                        )
                    except Exception as e:
            ...
            logger.error(
                f"[ROLE_SELECTION] ❌ Ошибка при создании записей в БД: {str(e)}"
            )
            await callback.message.edit_text(
        ...
        await callback.answer(
            "❌ Ошибка при обработке выбора роли. Пожалуйста, попробуйте позже."
        )
...
async def get_accounts(user_id: int) -> List[Dict[str, Any]]:
    ...
    try:
        ...
        try:
            ...
            fb_accounts = await client.get_ad_accounts()
            logger.debug(
                f"[GET_ACCOUNTS] ✅ Получено {len(fb_accounts)} аккаунтов из Facebook API"
            )
            ...
            if user.role == "owner":
                ...
                for account_data in fb_accounts:
                    ...
                    existing = (
                        session.query(Account).filter_by(fb_account_id=account_id).all()
                    )
                    ...
                    for acc in existing:
                        ...
                        logger.debug(
                            f"[GET_ACCOUNTS] 🔄 Обновлен аккаунт: {account_id} с именем {account_name}"
                        )
                    ...
                    if not any(acc.telegram_id == user_id for acc in existing):
                        ...
                        logger.debug(
                            f"[GET_ACCOUNTS] ➕ Добавлен новый аккаунт: {account_id} с именем {account_name}"
                        )
                ...
                logger.debug(
                    f"[GET_ACCOUNTS] 📋 Найдено {len(user_accounts)} назначенных аккаунтов в БД"
                )
                        ...
                        result.append(
                            {
                                "id": account.fb_account_id,
                                "name": account.name
                                or f"Аккаунт {account.fb_account_id}",
            ...
            logger.error(
                f"[GET_ACCOUNTS] ❌ Ошибка при получении аккаунтов из Facebook API: {str(e)}"
            )
            if user.role != "owner":
...
async def cmd_delete_role(message: Message):
    ...
    logger.info(
        f"[DELETE_ROLE] 🗑 Получена команда /delete_role от пользователя {user_id}"
    )
    ...
    try:
        ...
        if len(args) != 2:
            logger.warning(
                f"[DELETE_ROLE] ⚠️ Неверный формат команды от пользователя {user_id}"
            )
            await message.answer(
        ...
        try:
            ...
            await message.answer(
                "❌ Неверный формат ID пользователя. Используйте только цифры."
            )
            return
        ...
        if user.role == Role.OWNER.value:
            logger.warning(
                f"[DELETE_ROLE] ⚠️ Попытка удаления owner'а: {target_user_id}"
            )
        ...
        logger.error(
            f"[DELETE_ROLE] ❌ Ошибка при удалении пользователя {target_user_id}: {str(e)}"
        )
        session.rollback()
...
async def cmd_list_users(message: Message):
    ...
    try:
        ...
        for user in users:
            ...
            accounts = (
                session.query(Account).filter_by(telegram_id=user.telegram_id).all()
            )
            ...
            if accounts:
                ...
                for account in accounts:
                    ...
                    account_name = (
                        account.name[:27] + "..."
                        if account.name and len(account.name) > 30
                        else account.name or "Без имени"
                    )
