...
from src.storage.models import Account, Cache, User
from src.utils.export import (export_data_to_csv, export_data_to_excel,
                              export_data_to_json)
from src.utils.languages import fix_user_id, get_language, get_text
from src.utils.logger import get_logger
from src.utils.message_formatter import format_campaign_table, format_insights
...
async def stats_callback(callback: CallbackQuery):
    ...
    if user_id == 8113924050 or str(user_id) == "8113924050":
        ...
        try:
            ...
            if user:
                print(
                    f"DEBUG: Replacing bot ID with user ID in stats callback: {user.telegram_id}"
                )
    ...
    try:
        await callback.message.edit_text(
            get_text("loading_stats", lang), parse_mode="HTML"
        )
    ...
    try:
        ...
        if object_type == "account":
            ...
            if not campaigns:
                ...
                builder.add(
                    InlineKeyboardButton(
                        text=get_text("main_menu", lang), callback_data="menu:main"
                    )
                )
                button_count += 1
                ...
                if button_count % 2 != 0:
                    builder.add(
                        InlineKeyboardButton(text=" ", callback_data="empty:action")
                    )
            ...
            for campaign in campaigns:
                ...
                if campaign_id:
                    try:
                        campaign_insights = await client.get_campaign_insights(
                            campaign_id, date_preset
                        )
                        ...
                        logger.warning(
                            f"Error getting insights for campaign {campaign_id}: {str(e)}"
                        )
        ...
        if not insights:
            ...
            await callback.message.edit_text(
                get_text(
                    "no_stats_found",
                    lang,
                    object_type=get_text(object_type.replace("_campaigns", ""), lang),
                ),
                reply_markup=builder.as_markup(),
            )
            return
        ...
        if object_type == "account_campaigns":
            ...
            formatted_text = format_campaign_table(
                campaigns, insights, date_preset, user_id
            )
        else:
            formatted_text = format_insights(
                insights, object_type, date_preset, user_id
            )
        ...
        if object_type != "account_campaigns":
            ...
            if formatted_text.startswith(
                f"<b>{get_text('insights_for', lang, type=obj_type_display, name='')}</b>"
            ):
        ...
        if object_type == "account":
            ...
            builder.add(
                InlineKeyboardButton(
                    text="⬅️", callback_data=f"menu:account:{object_id}"
                )
            )
        else:
            builder.add(
        ...
        # Add empty button for even grid if needed
        if button_count % 2 != 0:
            builder.add(InlineKeyboardButton(text=" ", callback_data="empty:action"))
        ...
        await callback.message.edit_text(
            formatted_text, parse_mode="HTML", reply_markup=builder.as_markup()
        )
        ...
        builder.add(
            InlineKeyboardButton(
                text=get_text("back_to_accounts", lang), callback_data="menu:accounts"
            )
        )
        button_count += 1
    ...
    try:
        ...
        if menu_item == "main":
            ...
            try:
                ...
                logger.warning(
                    f"Could not delete message when going to accounts: {str(e)}"
                )
            ...
            try:
                ...
                loading_message = await callback.bot.send_message(
                    chat_id, "🔄 Загружаем список ваших рекламных аккаунтов..."
                )
            ...
            if not has_permission(user.role, Permission.MANAGE_USERS.value):
                await callback.message.edit_text(
                    "❌ У вас нет прав для выполнения этой команды.", parse_mode="HTML"
                )
                return
            ...
            await callback.message.edit_text(
                "🌐 Language", parse_mode="HTML", reply_markup=build_language_keyboard()
            )

        elif menu_item == "notifications":
            # Show notifications menu
            from src.bot.notification_handlers import (
                build_notification_keyboard, format_notification_settings)
            from src.storage.models import NotificationSettings

            # Получаем текущие настройки уведомлений
            settings = (
                session.query(NotificationSettings).filter_by(user_id=user_id).first()
            )
        ...
        try:
            await callback.message.edit_text(
                f"❌ Произошла ошибка: {str(e)}", parse_mode=None
            )
        except:
            pass
    finally:
...
async def account_campaigns_stats_callback(callback: CallbackQuery):
    ...
    if len(parts) < 2:
        await callback.message.edit_text(
            "❌ Неверный формат запроса статистики кампаний."
        )
        return
    ...
    try:
        ...
        if not campaigns:
            ...
            builder.add(
                InlineKeyboardButton(
                    text="⬅️", callback_data=f"menu:account:{account_id}"
                )
            )
            ...
            await callback.message.edit_text(
                get_text("no_campaigns_found", lang), reply_markup=builder.as_markup()
            )
            return
        ...
        try:
            await callback.message.edit_text(
                "📅 Выберите период:",
                parse_mode="HTML",
                reply_markup=build_date_preset_keyboard(
                    account_id, "account_campaigns", account_name
                ),
            )
        except TelegramBadRequest as e:
            # Message was deleted or can't be edited
            ...
            try:
                ...
                logger.error(
                    f"Failed to show date selection keyboard: {str(text_error)}"
                )
                await callback.message.edit_text(
                    "❌ Не удалось отобразить выбор периода."
                )
        ...
        builder.add(
            InlineKeyboardButton(text="⬅️", callback_data=f"menu:account:{account_id}")
        )
...
async def account_callback(callback: CallbackQuery):
    ...
    account_id = parts[1]
    print(
        f"DEBUG: Process account callback - Account ID: {account_id}, User ID: {callback.from_user.id}"
    )
...
async def account_menu_callback(callback: CallbackQuery):
    ...
    builder.add(
        InlineKeyboardButton(
            text="📊 Статистика", callback_data=f"account_campaigns_stats:{account_id}"
        )
    )

    # Main menu button
    builder.add(InlineKeyboardButton(text="🌎 Меню", callback_data="menu:main"))

    # Back to accounts list button
    builder.add(InlineKeyboardButton(text="⬅️", callback_data="menu:accounts"))
    ...
    # Send the menu
    await callback.message.edit_text(
        f"<b>{account_name}</b>", reply_markup=builder.as_markup(), parse_mode="HTML"
    )
