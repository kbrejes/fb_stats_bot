...
async def cmd_campaigns(message: Message, command: CommandObject):
    ...
    await message.answer(
        f"🔄 Загружаем список кампаний для аккаунта {account_id}...", parse_mode=None
    )
...
async def process_campaign_callback(callback: CallbackQuery):
    ...
    if not is_valid:
        ...
        try:
            ...
            builder.add(InlineKeyboardButton(text="⬅️", callback_data="menu:campaigns"))
            button_count += 1
    ...
    try:
        await callback.message.edit_text(
            f"🔄 Загружаем список объявлений для кампании {campaign_id}...",
            parse_mode=None,
        )
    except Exception as e:
    ...
    try:
        ...
        builder.add(
            InlineKeyboardButton(
                text="↩️ Назад к кампаниям", callback_data="menu:campaigns"
            )
        )
        button_count += 1
...
async def process_campaigns(callback: CallbackQuery, account_id: str, user_id: int):
    ...
    """
    print(
        f"DEBUG: process_campaigns started - Account ID: {account_id}, User ID: {user_id}"
    )
    try:
        ...
        for i in range(1, len(campaign_parts)):
            try:
                print(
                    f"DEBUG: Sending additional campaign data part {i+1} of {len(campaign_parts)}"
                )
                ...
                print(
                    f"DEBUG: Markdown error in additional parts: {str(markdown_error)}"
                )
                # Try without parse_mode if markdown fails
        ...
        # Handle API errors
        logger.error(
            f"Facebook API error in process_campaigns: {e.message} (code: {e.code})"
        )
        print(
            f"DEBUG: Facebook API error in process_campaigns - Message: {e.message}, Code: {e.code}"
        )
