...
from src.bot.keyboards import build_language_keyboard, build_main_menu_keyboard
from src.storage.database import get_session
from src.storage.models import NotificationSettings, User
from src.utils.bot_helpers import fix_user_id
from src.utils.languages import SUPPORTED_LANGUAGES, get_language, set_language
...
async def process_language_callback(callback: CallbackQuery):
    ...
    if language_code in SUPPORTED_LANGUAGES:
        ...
        await callback.message.edit_text(
            "❌ Error: Invalid language code.", reply_markup=build_main_menu_keyboard()
        )
...
async def process_menu_campaign_callback(callback: CallbackQuery):
    ...
    try:
        await callback.message.edit_text(
            f"🔄 Загружаем список кампаний для аккаунта {account_id}...",
            parse_mode=None,
        )
    except Exception as e:
