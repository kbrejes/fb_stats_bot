...
async def cmd_ads(message: Message, command: CommandObject):
    ...
    await message.answer(
        f"🔄 Загружаем список объявлений для кампании {campaign_id}...", parse_mode=None
    )
...
async def process_ad_callback(callback: CallbackQuery):
        ...
        try:
            ...
            builder.add(
                InlineKeyboardButton(
                    text="↩️ Назад к объявлениям", callback_data="menu:ads"
                )
            )
            button_count += 1
    ...
    try:
        await callback.message.edit_text(
            f"🔄 Загружаем статистику для объявления {ad_id}...", parse_mode=None
        )
    except Exception as e:
    ...
    try:
        ...
        if not insights:
            await callback.message.edit_text(
                "⚠️ Не найдена статистика для указанного объявления.", parse_mode=None
            )
            return
        ...
        builder.add(
            InlineKeyboardButton(text="↩️ Назад к объявлениям", callback_data="menu:ads")
        )
        button_count += 1
