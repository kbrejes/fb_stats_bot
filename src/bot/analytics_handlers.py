...
async def handle_analytics_callback(callback: CallbackQuery):
    ...
    try:
        ...
        if action == "menu":
            ...
            builder.add(
                InlineKeyboardButton(
                    text="Вчера vs Позавчера", callback_data="period:daily"
                )
            )
            builder.add(
                InlineKeyboardButton(
                    text="Эта vs Прошлая неделя", callback_data="period:weekly"
                )
            )
            builder.add(
                InlineKeyboardButton(
                    text="Прошлая vs Позапрошлая", callback_data="period:prev_weekly"
                )
            )
            builder.add(
                InlineKeyboardButton(
                    text="2 Недели vs Месяц", callback_data="period:biweekly_vs_monthly"
                )
            )
            ...
            await callback.message.edit_text(
                "📊 Выберите период для сравнения:", reply_markup=builder.as_markup()
            )
        ...
        await callback.message.edit_text(
            ...
            .add(InlineKeyboardButton(text="⬅️ Назад", callback_data="menu:main"))
            .as_markup(),
        )
...
async def handle_period_selection(callback: CallbackQuery):
    ...
    try:
        ...
        await callback.message.edit_text(
            "🗿 Выберите аккаунт для анализа:", reply_markup=builder.as_markup()
        )
        ...
        await callback.message.edit_text(
            ...
            reply_markup=InlineKeyboardBuilder()
            .add(InlineKeyboardButton(text="⬅️ Назад", callback_data="analytics:menu"))
            .as_markup(),
        )
    except Exception as e:
...
async def handle_comparison(callback: CallbackQuery):
    ...
    try:
        ...
        await callback.message.edit_text(
            message_parts[0], reply_markup=builder.as_markup()
        )
...
async def handle_details(callback: CallbackQuery):
    ...
    try:
        ...
        builder.add(
            InlineKeyboardButton(
                text="⬅️ Назад", callback_data=f"compare:{period_type}:{account_id}"
            )
        )
        builder.adjust(1)
...
async def handle_analysis(callback: CallbackQuery):
    ...
    try:
        ...
        await callback.message.edit_text(
            "🤖 Анализирую данные с помощью OpenAI...", reply_markup=None
        )
        ...
        current_insights, previous_insights = (
            await analytics_service.get_comparative_insights(
        ...
        analysis = await analytics_service.analyze_insights(
            current_insights, previous_insights, account_name
        )
