...
async def cmd_accounts(message: Message):
    ...
    try:
        ...
        if not accounts:
            logger.warning(
                f"[ACCOUNTS] ⚠️ Не найдены аккаунты для пользователя {user_id}"
            )
            await message.answer(
        ...
        try:
            ...
            logger.debug(
                f"[ACCOUNTS] 📱 Отправлена клавиатура с {len(accounts)} аккаунтами"
            )

        except Exception as e:
            logger.error(f"[ACCOUNTS] ❌ Ошибка при создании клавиатуры: {str(e)}")
            await message.answer(
                f"⚠️ Ошибка при создании клавиатуры: {str(e)}", parse_mode=None
            )
...
async def process_account_callback(callback: CallbackQuery):
    ...
    print(
        f"DEBUG: Process account callback - Account ID: {account_id}, User ID: {user_id}"
    )
        ...
        try:
            ...
            builder.add(
                InlineKeyboardButton(
                    text="↩️ Назад к аккаунтам", callback_data="menu:accounts"
                )
            )
            button_count += 1
