    ...
    for account in accounts:
        ...
        builder.add(
            InlineKeyboardButton(
                text=account_name, callback_data=f"account:{account_id}"
            )
        )
        button_count += 1
...
def build_campaign_keyboard(campaigns: List[Dict], add_stats: bool = False):
    ...
    for campaign in campaigns:
        ...
        builder.add(
            InlineKeyboardButton(
                text=campaign_name, callback_data=f"campaign:{campaign_id}"
            )
        )
        button_count += 1
...
def build_ad_keyboard(ads: List[Dict], campaign_id: str, add_stats: bool = False):
    ...
    for ad in ads:
        ...
        builder.add(InlineKeyboardButton(text=ad_name, callback_data=f"ad:{ad_id}"))
        button_count += 1
        ...
        if add_stats:
            builder.add(
                InlineKeyboardButton(
                    text="📊 Статистика", callback_data=f"ad_stats:{ad_id}:{ad_name}"
                )
            )
            button_count += 1
    ...
    builder.add(
        InlineKeyboardButton(text="⬅️", callback_data=f"menu:campaign:{campaign_id}")
    )
    button_count += 1
...
def build_date_preset_keyboard(
    object_id: str, object_type: str, object_name: str = None
):
    """
    ...
    for group_name, presets in date_groups.items():
        for preset, label in presets.items():
            ...
            if len(callback_data) > 64:
                ...
                if (
                    len(id_part) > 30
                ):  # Оставляем запас для остальной части callback_data
                    id_part = id_part[:30]
    ...
    if object_type == "campaign":
        ...
        back_button_data = (
            f"menu:campaign:{object_id.split('_')[0]}"  # Get parent campaign ID
        )

    builder.add(InlineKeyboardButton(text="⬅️", callback_data=back_button_data))
    button_count += 1
...
def build_export_format_keyboard(data_key: str):
    ...
    for format in EXPORT_FORMATS:
        builder.add(
            InlineKeyboardButton(
                text=format.upper(), callback_data=f"export:{data_key}:{format}"
            )
        )

    # Back button
    builder.add(InlineKeyboardButton(text="⬅️", callback_data="back:cancel"))
...
def build_main_menu_keyboard(user_role: str = None):
    ...
    if has_permission(user_role, Permission.VIEW_STATISTICS):
        builder.add(
            InlineKeyboardButton(text="📈 Аналитика", callback_data="analytics:menu")
        )
        button_count += 1
    ...
    if has_permission(user_role, Permission.MANAGE_USERS):
        builder.add(
            InlineKeyboardButton(text="🪓 Авторизация", callback_data="menu:auth")
        )
        button_count += 1

    # Language button
    builder.add(InlineKeyboardButton(text="🪆 Язык", callback_data="menu:language"))
    button_count += 1
...
def build_confirmation_keyboard(action: str, object_id: str):
    ...
    builder.add(
        InlineKeyboardButton(
            text="✅ Подтвердить", callback_data=f"confirm:{action}:{object_id}"
        )
    )
...
def build_language_keyboard():
    ...
    builder.add(InlineKeyboardButton(text="🇷🇺 Русский", callback_data="language:ru"))

    builder.add(InlineKeyboardButton(text="🇬🇧 English", callback_data="language:en"))

    # Add back button
    builder.add(InlineKeyboardButton(text="⬅️", callback_data="menu:main"))
