...
class DataProcessor:
        ...
        if all(col in df.columns for col in selected_columns):
            ...
            if "Статус" in df.columns:
                df["Статус"] = df["Статус"].map(
                    lambda x: status_map.get(x, f"Неизвестный ({x})")
                )
            ...
            return "\n".join(
                [
                    f"{account.get('name', 'Без имени')} ({account.get('id', 'Без ID')})"
    ...
    def format_campaigns(campaigns: List[Dict]) -> str:
        ...
        if all(col in df.columns for col in selected_columns):
            ...
            return "\n".join(
                [
                    f"{campaign.get('name', 'Без имени')} ({campaign.get('id', 'Без ID')})"
    ...
    def format_ads(ads: List[Dict]) -> str:
        ...
        if all(col in df.columns for col in selected_columns):
            ...
            return "\n".join(
                [
                    f"{ad.get('name', 'Без имени')} ({ad.get('id', 'Без ID')})"
                    for ad in ads
                ]
            )
    ...
    @staticmethod
    def format_insights(
        insights: List[Dict], account_name: str = "Без имени"
    ) -> Optional[str]:
        """
        ...
        logger.info(
            f"Calculated conversion cost for {account_name}: {conversion_cost} (spend: {total_spend}, conversions: {conversions})"
        )
        ...
        message = [
            ...
            (
                f"Цена конверсии: ${conversion_cost:.2f}"
                if conversion_cost is not None
                else "Цена конверсии: н/д"
            ),
    ...
    def format_comparative_insights(
        ...
        try:
            current_start = datetime.strptime(
                current_insights[0]["date_start"], "%Y-%m-%d"
            )
            current_end = datetime.strptime(
                current_insights[0]["date_stop"], "%Y-%m-%d"
            )
            previous_start = datetime.strptime(
                previous_insights[0]["date_start"], "%Y-%m-%d"
            )
            previous_end = datetime.strptime(
                previous_insights[0]["date_stop"], "%Y-%m-%d"
            )
        ...
        # Получаем метрики
        current_metrics = DataProcessor.get_metrics(
            current_insights, normalize_by_days, current_days
        )
        previous_metrics = DataProcessor.get_metrics(
            previous_insights, normalize_by_days, previous_days
        )
    ...
    def convert_to_dataframe(
        data: List[Dict], level: str = "campaign", date_field: str = "date_start"
    ) -> pd.DataFrame:
        """
