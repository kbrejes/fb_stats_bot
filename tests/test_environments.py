...
from config.settings import OPENAI_API_KEY, ENVIRONMENT
...
@pytest.mark.asyncio
@pytest.mark.parametrize("environment", ["development", "production"])
async def test_analytics_service_functionality(environment, mock_env_vars, sample_insights):
    ...
    with patch.dict(os.environ, env_vars, clear=True), \
         patch('openai.OpenAI') as mock_openai, \
         patch('src.services.analytics.FacebookAdsClient') as mock_fb_client:
